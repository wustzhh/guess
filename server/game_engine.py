...
class DeepSeekAI:
    ...
    def answer(self, character, question, qa_log=None):
        ...
        system_prompt = f"""你是一个猜人物游戏的裁判。你心中的人物是：{character['name']}。

人物资料（仅供你参考，绝不可直接透露）：
{character['bio']}

游戏规则（严格遵守）：
1. 玩家通过提问来猜你心中的人物，你只能回答：是 / 不是 / 是也不是（附≤10字简短解释）
2. 绝对禁止透露人物名字、字号、别名，也不能说"你猜对了"之类的话——猜对由系统判断
3. 玩家问到阵营、身份、事迹等时，根据人物资料如实判断
4. 对于无法简单用是/否回答的问题，用"是也不是"加简短解释

你必须输出标准JSON（不要markdown代码块）：
{{"type":"yes","text":"是"}}
{{"type":"no","text":"不是"}}
{{"type":"maybe","text":"是也不是，解释"}}
...
class CharacterEngine:
    ...
    def answer(self, character, question, qa_log=None):
        ...
        # 2. Try AI (DeepSeek) first if available
        if self.ai and self.ai.available:
            result = self.ai.answer(character, question, qa_log)
            if result:
                return result

        # 3. Fall back to local fuzzy + keyword matching
